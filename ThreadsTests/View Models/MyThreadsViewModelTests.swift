...
final class MyThreadsViewModelTests: ViewModelTestCase {
    ...
    func testShowEmptyView() throws {
        ...
        wait(for: [expectation(for: NSPredicate(format: "showEmptyView = YES"), evaluatedWith: fakeView)], timeout: 3.0)
        ...
        wait(for: [expectation(for: NSPredicate(format: "showEmptyView = NO"), evaluatedWith: fakeView)], timeout: 3.0)
    }

    func testSnapshot() throws {
        XCTAssertNil(fakeView.snapshot)

        subject.snapshot.map { s -> MyThreadsViewModel.Snapshot? in s }.assign(to: \.snapshot, on: fakeView).store(in: &cancellables)

        wait(for: [expectation(for: NSPredicate(format: "hasSnapshot = YES and threadCount = 0"), evaluatedWith: fakeView)], timeout: 3.0)
        ...
        wait(for: [expectation(for: NSPredicate(format: "hasSnapshot = YES and threadCount = 1"), evaluatedWith: fakeView)], timeout: 3.0)
