...
extension Project {
    ...
    @objc dynamic var formattedNotes: NSAttributedString? {
        get {
            (notes ?? NSAttributedString()).replacing(
                font: .preferredFont(forTextStyle: .body), color: .label)
        }
        set {
            notes
                = (newValue ?? NSAttributedString()).replacing(
                    font: .preferredFont(forTextStyle: .body), color: .label)
        }
    }
    ...
    private func collectRecordsToUpdate(projectRecord: CKRecord) throws -> (
        [CKRecord], [ProjectImage], [ProjectThread]
    ) {
        ...
        let projectThreads = try self.managedObjectContext!.fetch(
            ProjectThread.fetchRequest(for: self))
