...
class StoreObserver: NSObject, SKPaymentTransactionObserver {
    ...
    init(productIDs: Set<StoreProduct>) {
        ...
        self.purchasedProductIDs
            = Set(productIDs.filter { UserDefaults.standard.bool(forKey: $0.rawValue) })
    }
    ...
    func attemptPurchase(
        _ product: SKProduct, completionHandler: @escaping (Result<(), Error>) -> Void
    ) {
        let payment = SKPayment(product: product)
        let observer = PurchaseObserver(
            productID: product.productIdentifier, completionHandler: completionHandler)
    ...
    private func handleSuccessfulPurchase(_ transaction: SKPaymentTransaction) {
        guard let product = StoreProduct(rawValue: transaction.payment.productIdentifier) else {
            return
        }
